...
function App() {
  ...
  return (
    <div>
      <p className='text-4xl font-bold text-center my-10 text-blue-500 underline'>Try Your Luck / Mind</p>
      <div className='w-1/2 mx-auto flex flex-col gap-3 rounded-xl shadow-lg py-8 px-12'>
        <p className='font-semibold text-3xl font-bold text-red-700 font-bold'>Question {index + 1}</p>
        ...
        <div className='flex flex-col gap-2'>
            ...
            questions[index].incorrect_answers.map((option, idx) => {
              return (
                <p
                  ...
                  className="option bg-pink-500 text-white py-3 px-5 cursor-pointer hover:bg-white hover:text-black rounded-lg"
                >
                  {option}
                </p>
              );
          ...
          <p 
            ...
            className='option bg-pink-500 text-white py-3 px-5 cursor-pointer hover:bg-white hover:text-black rounded-lg'>{questions[index].correct_answer}
          </p>
        </div>
        <p>Time Left: {timeLeft} seconds</p>
        <button 
          onClick={() => nextQuestion(null)}
          className='bg-blue-500 text-white font-semibold hover:bg-blue-600 w-[25%] py-2 rounded-lg font-semibold mx-auto'
        >
          Skip Question
        </button>
      </div>
